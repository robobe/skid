...
bool ArduPilotPlugin::InitArduPilotSockets(sdf::ElementPtr _sdf) const
{
  ...
  this->dataPtr->fdm_port_in = 9012;
    // _sdf->Get<uint16_t>("fdm_port_in", static_cast<uint16_t>(9002)).first;
  this->dataPtr->fdm_port_out = 9013;
    // _sdf->Get("fdm_port_out", static_cast<uint16_t>(9003)).first;

  gzmsg << "Start Plugin on port " << this->dataPtr->fdm_port_in << "\n";
  ...
  gzmsg << "Connect Plugin to port " << this->dataPtr->fdm_port_out << "\n";
